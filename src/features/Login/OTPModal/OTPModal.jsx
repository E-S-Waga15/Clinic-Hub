...
const OTPModal = ({ isOpen, onClose, onVerifyCode }) => {
  ...
  const { phoneNumber, loading, isLoggedIn, filledData, error } = useSelector(
    (state) => state.auth
  );
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "Backspace") {
      if (otp.length > 0) {
        const updatedOtp = otp.slice(0, -1);
        setOtp(updatedOtp);
        setFocusedIndex(Math.max(updatedOtp.length, 0));
      }
      e.preventDefault();
    }
  };
  ...
  const handleChange = (e) => {
    const raw = e.target.value.replace(/\D/g, '').slice(0, 4); // أرقام فقط وطول 4
    const fixed = raw.split('').join(''); // إجبار React على التحديث بدقة
    setOtp(fixed);
    setFocusedIndex(Math.min(fixed.length, 3));
  };
        ...
        <div className="row h-100 flex-column flex-lg-row">
          <div className="otp-content-container col-12 col-lg-6">
                    ...
                    .map((_, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isActive && !isFilled && (
                            <div className="otp-cursor-blink"></div>
                          )}
                        </div>
                      );
                    })}
            ...
            <p className="mt-3 text-center">
              ...
              <button className="btn btn-link p-0 otp-resend-link">
                Resend Code
              </button>
            </p>
          </div>
